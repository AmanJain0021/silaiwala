  ...
  try {
    ...
    if (earnedAmount <= 0) {
      ...
      if (profile?.currentLocation?.coordinates?.length === 2 && request.customerLocation?.coordinates?.length === 2) {
        const [lon1, lat1] = profile.currentLocation.coordinates;
        const [lon2, lat2] = request.customerLocation.coordinates;
        const { getRoadDistanceKm } = require("../../../utils/checkoutPricing.js");
        distance = await getRoadDistanceKm(lat1, lon1, lat2, lon2);
      }
...
exports.getDashboardStats = asyncHandler(async (req, res, next) => {
  ...
  let profile = await MeasurementExecutive.findOne({ user: req.user.id }).populate("user", "name email phoneNumber profileImage");
  if (!profile) {
    ...
    profile = await MeasurementExecutive.findById(profile._id).populate("user", "name email phoneNumber profileImage");
  }

  const [totalPending, totalAccepted, completedToday, totalCompleted] =
    await Promise.all([
