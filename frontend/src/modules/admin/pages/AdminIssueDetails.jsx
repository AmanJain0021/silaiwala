...
const AdminIssueDetails = () => {
    ...
    const handleForceResolve = async () => {
        if (!window.confirm("Force resolve this issue?")) return;
        try {
            const res = await api.patch(`/issues/admin/${issueId}/status`, { status: 'resolved' });
            setIssue(res.data.data);
    ...
    if (!issue) return <div className="flex h-[100dvh] items-center justify-center text-gray-500 font-medium">Issue not found</div>;
    ...
    return (
        <div className="flex flex-col h-[100dvh] pb-[76px] md:pb-0 bg-gray-50 relative">
            <div className="bg-white border-b border-gray-100 p-4 flex items-center gap-4 sticky top-0 z-10 shadow-sm">
                <button onClick={() => navigate('/admin/issues')} className="w-10 h-10 flex items-center justify-center bg-gray-50 rounded-full text-gray-900 active:scale-95 transition-transform">
                    <ChevronLeft size={20} />
                </button>
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                        Status: <span className="text-primary">{issue?.status?.replace(/_/g, ' ') || 'Unknown'}</span>
                    </p>
                </div>
            </div>
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-5">
                    {issue.status !== 'resolved' && issue.status !== 'closed' && (
                        <div className="flex gap-2 mb-5">
                            <button onClick={handleForceResolve} className="w-full bg-green-100 text-green-700 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-wide flex items-center justify-center gap-1.5 active:scale-95 transition-transform border border-green-200">
                                <CheckCircle size={16} /> Force Resolve Issue
                            </button>
                        </div>
                    )}
                    ...
                    messages.map((msg, i) => {
                        ...
                        return (
                            <div key={i} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] rounded-3xl p-4 shadow-sm ${isAdmin ? 'bg-primary text-white rounded-br-sm shadow-primary/10' : 'bg-white border border-gray-100 text-gray-900 rounded-bl-sm'}`}>
                                    {msg.imageUrl && (
                                    ...
                                    <p className={`text-[9px] font-bold mt-2 tracking-wider uppercase ${isAdmin ? 'text-white/70' : 'text-gray-400'}`}>
                                        {!isAdmin && <span className="mr-1 opacity-75">{msg.sender?.name || msg.senderRole} • </span>}
