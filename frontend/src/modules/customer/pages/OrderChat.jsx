...
const OrderChat = () => {
    ...
    useEffect(() => {
        ...
        if (socket) {
            ...
            // Join the order room if not already joined (socket logic usually handles this in App.jsx or tracking, but just in case)
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                // Message will be added via socket, but we can optimistically add it or just wait
                setNewMessage('');
            }
        } catch (err) {
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
                <div className="bg-red-50 p-6 rounded-3xl text-center border border-red-100 max-w-sm">
                    ...
                    <button 
                        onClick={() => navigate(`/user/orders/${id}/track`)}
    ...
    return (
        <div className="flex flex-col h-screen bg-[#FDFBF7] font-sans relative">
            ...
            <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100 px-4 pt-6 pb-4 flex items-center gap-4 shadow-[0_8px_30px_-15px_rgba(0,0,0,0.05)]">
                <button onClick={() => navigate(`/user/orders/${id}/track`)} className="p-2 -ml-2 rounded-full hover:bg-gray-50 active:bg-gray-100 text-gray-700 transition-colors">
                    <ArrowLeft size={22} />
                </button>
                <div className="flex-1">
                    <h1 className="text-base font-black text-gray-900 leading-tight">Chat with Artisan</h1>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5 z-10">
                ...
                {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-40 text-gray-400 space-y-3 opacity-60">
                        ...
                        <p className="text-xs font-semibold">Say hello to your artisan!</p>
                    </div>
                ) : (
