...
import { Lock, KeyRound, User, Eye, EyeOff, ArrowLeft, CheckCircle2 } from 'lucide-react';
...
const ForgotPassword = () => {
    ...
    const handleSendOtp = async (e) => {
        ...
        try {
            ...
            if (checkRes.data.role !== 'customer') {
                setError('This number is not registered as a Customer.');
                setSendingOtp(false);
                return;
            }
        ...
        >
            ...
            <div className="w-16 h-16 rounded-[22px] bg-[#F4EFFF] border border-[#E9DFFE] flex items-center justify-center shadow-2xs mb-4 shrink-0 mx-auto mt-2">
                {step === 'reset' ? (
                    ...
                    <Lock className="w-6 h-6 text-[#843D9B]" strokeWidth={2.2} />
                )}
            </div>
            ...
            <div className="text-center mb-6 w-full">
                <h1 className="text-2xl sm:text-[26px] font-bold text-[#0F172A] tracking-tight mb-1">
                    {step === 'request' ? 'Forgot Password' : step === 'verify' ? 'Verify OTP' : 'Set New Password'}
                </h1>
