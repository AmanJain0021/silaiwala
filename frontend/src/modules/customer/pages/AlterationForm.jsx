        ...
        try {
            ...
            const response = await api.post('/customers/cart', {
                ...
                config: {
                    ...
                    alterationImages: validUrls
                }
            });

            if (response.data.success) {
                // Instantly submit the request directly from the cart
                const altRes = await api.post('/alterations/request', {
                    deliveryAddress: {
                        street: 'Customer Home', // Alteration pickup handled later or uses profile address
                        city: 'Auto',
                        zipCode: 'Auto',
                ...
                if (altRes.data.success) {
                    toast.success("Alteration request submitted to tailor!");
                    navigate('/user/orders', { state: { message: "Alteration request submitted successfully! Awaiting quote." } });
                } else {
                    toast.error("Added to cart, but failed to submit request.");
                    navigate('/user/cart');
                }
            }
        } catch (error) {
            console.error('Error adding to cart:', error);
            toast.error(error.response?.data?.message || "Failed to add to cart. Ensure cart is empty.");
        } finally {
            setIsSubmitting(false);
        }
    };
