...
import { ArrowLeft, Sparkles, Scissors, ShoppingBag, CheckCircle2, ChevronRight, Info, Loader2, Feather } from 'lucide-react';
...
const EmbroideryPage = () => {
    ...
    useEffect(() => {
        const fetchAddons = async () => {
            try {
                const res = await api.get('/style-addons?isActive=true&addonType=embroidery');
                if (res.data.success) {
            ...
            <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100">
                <div className="px-5 py-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button onClick={() => navigate('/user')} className="p-2 -ml-2 rounded-full hover:bg-gray-100 transition-all active:scale-90">
                            <ArrowLeft size={22} className="text-gray-900" />
                        </button>
                        <div>
                            <h1 className="text-lg font-black text-gray-900 leading-none">Embroidery Hub</h1>
                            <p className="text-[10px] text-primary font-bold uppercase tracking-widest mt-1">Custom Art for your outfits</p>
                        </div>
                    </div>
