        ...
        <div className="flex gap-4 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm animate-in slide-in-from-bottom-2">
            ...
            <div className="flex-1 flex flex-col justify-between">
                <div>
                    ...
                    {item.isAlteration ? (
                        <div className="text-[10px] text-gray-500 mt-1 line-clamp-2 italic">
                            {item.config?.alterationDescription}
                        </div>
                    ) : (
                ...
                <div className="flex justify-between items-end mt-2">
                    <div className={`font-bold ${item.isAlteration ? "text-amber-500 text-xs" : "text-[#843D9B]"}`}>
                        {item.isAlteration ? "Pending Quote" : `₹${item.price}`}
                    </div>
                    ...
                    {!item.isAlteration ? (
                        ...
                        <div className="text-[10px] font-bold text-[#843D9B] bg-indigo-50 px-2 py-1 rounded">
                            Qty: 1
                        </div>
                    )}
                </div>
